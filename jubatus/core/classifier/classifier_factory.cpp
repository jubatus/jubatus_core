...
shared_ptr<classifier_base> classifier_factory::create_classifier(
  ...
  const detail::nearest_neighbor_classifier_config* nconf =
      dynamic_cast<const detail::nearest_neighbor_classifier_config*>(conf_base);
	{	 // unlearner
		if (uconf) {
	...
	} else if (conf_base->method_ == "PA" || conf_base->method_ == "passive_aggressive") {
		// passive_aggressive doesn't have parameter
    JUBATUS_ASSERT(uconf != NULL);
		res.reset(new passive_aggressive());
	} else if (conf_base->method_ == "PA1" || conf_base->method_ == "passive_aggressive_1") {
    JUBATUS_ASSERT(uconf != NULL);
		res.reset(new passive_aggressive_1(uconf->regularization_weight));
	} else if (conf_base->method_ == "PA2" || conf_base->method_ == "passive_aggressive_2") {
    JUBATUS_ASSERT(uconf != NULL);
		res.reset(new passive_aggressive_2(uconf->regularization_weight));
	} else if (conf_base->method_ == "CW" || conf_base->method_ == "confidence_weighted") {
    JUBATUS_ASSERT(uconf != NULL);
		res.reset(new confidence_weighted(uconf->regularization_weight));
	} else if (conf_base->method_ == "AROW" || conf_base->method_ == "arow") {
    JUBATUS_ASSERT(uconf != NULL);
		res.reset(new arow(uconf->regularization_weight));
	} else if (conf_base->method_ == "NHERD" || conf_base->method_ == "normal_herd") {
    JUBATUS_ASSERT(uconf != NULL);
		res.reset(new normal_herd(uconf->regularization_weight));
	} else if (conf_base->method_ == "NN" || conf_base->method_ == "nearest_neighbor") {
